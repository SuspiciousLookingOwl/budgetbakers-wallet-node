...
export class Wallet {
    ...
    public getLabel(id: string): Label | null {
        const doc = this.docs.get(id) as DocumentType;
        return doc?.reservedModelType === "HashTag" ? doc : null;
    }

    public labels(): Label[] {
        const labels = [...this.docs.values()].filter(
            (d) => d.reservedModelType === "HashTag");
        return labels;
    }
    ...
    public get plannedPayments(): PlannedPayment[] {
        const plannedPayments = [...this.docs.values()].filter(
            (d) => d.reservedModelType === "StandingOrder",
        );
        return plannedPayments;
    }

    public get shoppingLists(): ShoppingList[] {
        const shoppingLists = [...this.docs.values()].filter(
            (d) => d.reservedModelType === "ShoppingList",
        );
        return shoppingLists;
    }

    public get goals(): Goal[] {
        const goals = [...this.docs.values()].filter(
            (d) => d.reservedModelType === "Goal",
        );
        return goals;
    }
}
